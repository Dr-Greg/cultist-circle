...
export async function GET() {
  try {
    const { data: simplifiedData, timestamp } = await getCachedData(
      ...
      async () => {
        console.log(
          `[${new Date().toISOString()}] Fetching PVP items from GraphQL API`
        );
        ...
        if (!response.ok) {
          ...
          console.error(
            ...
            errorData.errors || "Failed to fetch PVP data"
          );
          throw new Error(
    ...
    if (simplifiedData.length === 0) {
      console.log(`[${new Date().toISOString()}] No PVP items available`);
